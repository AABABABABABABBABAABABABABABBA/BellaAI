...
export function getDefaultShell(platform: string): ShellConfig {
  if (platform === "win32") {
    ...
    // to cmd.exe when git-bash isn't installed. Override with BELLAI_BASH_PATH.
 ...
 *   1. `BELLAI_BASH_PATH` environment override
 *   2. Common install locations
 *   3. `where git` → resolve `<gitDir>/../../bin/bash.exe`
 * Returns null if not found.
 */
export function findGitBash(): string | null {
  const override = process.env.BELLAI_BASH_PATH;
